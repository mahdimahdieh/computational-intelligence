...
def load_and_preprocess_pytorch(positive_class=0, data_dir='./data'):
    transform = T.ToTensor()

    train_set = CIFAR10(root=data_dir, train=True, download=True)
    test_set  = CIFAR10(root=data_dir, train=False, download=True)

    x_train = train_set.data.astype(np.float32) / 255.0  # (50000, 32, 32, 3)
    y_train = np.array(train_set.targets, dtype=np.int64)  # (50000,)
    x_test  = test_set.data.astype(np.float32)  / 255.0   # (10000, 32, 32, 3)
...
X_train, y_train, X_test, y_test = load_and_preprocess_pytorch(positive_class=0)
